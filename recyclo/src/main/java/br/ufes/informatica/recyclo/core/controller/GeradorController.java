...
public class GeradorController extends JSFController {
	...
	/** TODO: generated by FrameWeb. Should be documented. */
	public void alterarGerador(Gerador gerador) {
		
		// Chama o método da classe de servico para alterar o gerador.
		boolean geradorAlterado = geradorService.alterarGerador(gerador);
		
		// Se o gerador foi alterado, exibe mensagem de sucesso.
		if (geradorAlterado == true) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Cadastro atualizado com sucesso!" + gerador.getUsuario().getEmail(), "Cadastro atualizado com sucesso!" + gerador.getUsuario().getEmail()));
		}
